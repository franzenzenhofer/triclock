...
function layersMatch(config: TrichronoConfig, layers: LayerVisibility): boolean {
  ...
  const g = config.triangles.gapLayers[0]?.visible ?? false;
  return w === layers.wedge && p === layers.primary && c === layers.cross && g === layers.gap;
}
...
export function applyDisplayMode(config: TrichronoConfig, name: DisplayModeName): void {
  ...
  const gap = config.triangles.gapLayers[0];
  ...
  if (gap) mut.triangles.gapLayers = [{ ...gap, visible: mode.layers.gap }];
}
